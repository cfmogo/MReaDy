        subroutine bkmp2_f8_switch(i,V,dV)
        use variables
        use constants
        use phys_parameters
        implicit none

        double precision, dimension(3)  ::rH3,dvdrH3
...
!        print*,'208'
       ...
       ! Switch function
        ...
        ! for the diatomic potencials
            ...
            call pothht_f14(r3,Vdiat,dVdiat)
...
!  _/              (also known as the bkmp2 surface)                 _/
!  _/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
!
!  All distances are in bohrs and all energies are in hartrees.
!  Note that it is an error if any distance is less than 0.2 bohrs.
!
!  ALL REAL VARIABLES ARE DOUBLE PRECISION (double precision)
!
!  For a discussion of this surface, see the BKMP2 paper:
!    "A Refined H3 Potential Energy Surface", A.I. Boothroyd,
!    W.J. Keogh, P.G. Martin, and M.R. Peterson, Journal of
!    Chemical Physics, 104, 7139-7152 (1996)
!  and the earlier BKMP paper:
!    "An Improved H3 Potential Energy Surface", A.I. Boothroyd,
!    W.J. Keogh, P.G. Martin, and M.R. Peterson, Journal of
!    Chemical Physics, 95, 4343-4359 (Sept15/1991)
!
!  As of 20 Sept 2003, additional interfaces to this surface have
!  been added, to make it easier to use various coordinate systems
!  (the surface itself remains unchanged, of course).
!
!######################################################################
!
!  r1 is the distance between the first two H atoms
!  r2 is the distance between the second and third H atoms
!  r3 is the distance between the first and third H atoms
!  RR is the distance between the third H atom and the H2 molecule (center)
!  g is the angle between r1 and Rr (in radians)
!  a2, y3, z3 are the minimal cartesian coordinates (that were used
!               when specifying the points fitted by the BKMP2 surface)
!
!                        z-axis          y = y3
!                       ^                |
!                       |                |
...
!######################################################################
!
!  Basic interface, as a function of interatomic distances:
!  ========================================================
!
!  call bkmp2_f8(r,ideriv,Vtot,dVtot)
!  -------------------------------
!
!  Input: r(3) = ( r1, r2, r3 ) , an array contining the three interatomic
!                   distances
!         ideriv = (integer) derivative flag: ideriv > 0 means "calculate
!                     the analytic first derivatives", ideriv = 0 means
!                     "do not do so (return zero for all derivatives)"
!
!  Output: Vtot = total H3 surface energy in hartrees,
!                   relative to separated H + H + H atoms
!          dVtot(3) = ( dVdr1, dVdr2, dVdr3 ) , an array containing the
!                        derivatives of the surface energy with respect
!                        to each interatomic distance (with the other
!                        two distances held fixed)
!
!######################################################################
!
!  Scalar interface, as a function of interatomic distances:
!  =========================================================
!
!  call bkmp2_r_f8(r1,r2,r3,ideriv,Vtot,dVdr1,dVdr2,dVdr3)
!  ----------------------------------------------------
!
!  The same as the basic interface, except that the distnaces and
!  derivatives are given as scalar variables instead of arrays.
!
!######################################################################
!
!  Angular interface:
!  ==================
!
!  call bkmp2_ang_f8(r1,RR,g,ideriv,V,dVdr1,dVdr2,dVdr3,r2,r3,dVdr1gR,dVdRR,dVdg)
!  ---------------------------------------------------------------------------
!
!  Input: r1 = distance between first two H atoms
!         RR = distance between the third H atom and the H2 molecule (center)
!         g = H2 - H angle in radians
!         ideriv = (integer) derivative flag
!
!  Output: V = total H3 surface energy in hartrees
!          dVdr1, dVdr2, dVdr3 = derivatives with respect to the
!                                  three interatomic distnaces
!          r2, r3 = the other two interatomic distances
!          dVdr1gR = derivative of V with respect to r1 at fixed RR and g
!                      (note that this is NOT equal to dVdr1, in general)
!          dVdRR = derivative of V with respect to RR at fixed r1 and g
!          dVdg = derivative of V with respect to g at fixed r1 and RR
!
!######################################################################
!
!  General cartesian interface, including cartesian derivatives:
!  =============================================================
!
!  call bkmp2_cc_f8(c1,c2,c3,ideriv,Vtot,dVdr1,dVdr2,dVdr3,r1,r2,r3,dc1,dc2,dc3)
!  --------------------------------------------------------------------------
!
!  Input: c1(3) = cartesian coordinates (x1,y1,z1) of first H-atom
!         c2(3) = cartesian coordinates (x2,y2,z2) of second H-atom
!         c3(3) = cartesian coordinates (x3,y3,z3) of third H-atom
!         ideriv = (integer) derivative flag
!
!  Output: Vtot = total H3 surface energy in hartrees
!          dVdr1, dVdr2, dVdr3 = derivatives with respect to the
!                                  three interatomic distnaces
!          r1, r2, r3 = the three interatomic distances
!          dc1(3) = derivatives (dVdx1,dVdy1,dVdz1) of V with respect to
!                    cartesian coordinates of first H-atom
!          dc2(3) = derivatives (dVdx2,dVdy2,dVdz2) of V with respect to
!                    cartesian coordinates of second H-atom
!          dc3(3) = derivatives (dVdx3,dVdy3,dVdz3) of V with respect to
!                    cartesian coordinates of third H-atom
!
!  Note: dVdx1,...,dVdz3 are the derivatives with all the other eight
!        cartesian coordinates held fixed in each case.
!
!######################################################################
!
!  Minimal cartesian interface (useful mainly for fitted geometries):
!  ==================================================================
!
!  call bkmp2_cc3_f8(a2,y3,z3,ideriv,Vtot,dVdr1,dVdr2,dVdr3,r1,r2,r3)
!  ---------------------------------------------------------------
!
!  Input: a2 = 0.5 * r1 = position on z-axis of the second H atom
!         y = y3 = distance of third H atom from z-axis
!         z = z3 = height of third H atom above x-y plane
!         ideriv = (integer) derivative flag
!
!  Output: Vtot = total H3 surface energy in hartrees
!          dVdr1, dVdr2, dVdr3 = derivatives with respect to the
...
!######################################################################
!
      subroutine surfid_f8(h3id)
!-----------------------------------------------------------------
!  h3 surface identification number (based on date of fitting)
!-----------------------------------------------------------------
...
!
      ...
      subroutine bkmp2_cc_f8(ccH1,ccH2,ccH3,ideriv,V,dVdr12,dVdr23, &
                  dVdr13,r12,r23,r13,dVdccH1,dVdccH2,dVdccH3)
!     -----------------------------------------------------------------
!
      implicit double precision (a-h,o-z)
      IMPLICIT INTEGER(I-N)
...
!
!__Get the three distances as a function of the coordinates.
!
...
!
      call bkmp2_r_f8(r12,r23,r13,ideriv,Vtot,dVdr12,dVdr23,dVdr13)
!
!__Get the derivatives as a function of the cartesian coordinates.
!
      if ( ideriv .gt. 0 ) then
!
...
!
      subroutine bkmp2_cc3_f8(a2,y3,z3,ideriv,Vtot, &
                           dVdr12,dVdr23,dVdr13,r12,r23,r13)
!-----------------------------------------------------------------
!
      implicit double precision (a-h,o-z)
      IMPLICIT INTEGER(I-N)
!__Get the three distances.
!
      r12 = 2.d0 * a2
...
!
      call bkmp2_r_f8(r12,r23,r13,ideriv,Vtot,dVdr12,dVdr23,dVdr13)
!
!__Derivatives as a function of the minimal cartesian coordinates
!  would not usually be useful; do not compute them.
!
      return
      end
!
      subroutine bkmp2_ang_f8(rhh,rh2h,gammarad,ideriv,Vtot,dVdrhh, &
...
!
!__Get the distances r() from rhh, rh2h, and the angle gammarad
!
      cgam = cos(gammarad)
      sgam = sin(gammarad)
      cgamR = cgam * rh2h
...
!
      call bkmp2_f8(r,Vtot,dVtot,ideriv)
!
      dVdrhh = dVtot(1)
      dVdr2 = dVtot(2)
      dVdr3 = dVtot(3)
!
!__Get the derivatives as a function of the angular coordinates.
!
      if ( ideriv .gt. 0 ) then
!
...
!
      subroutine bkmp2_r_f8(r1,r2,r3,ideriv,Vtot,dVdr1,dVdr2,dVdr3)
!-----------------------------------------------------------------
!
      implicit double precision (a-h,o-z)
      IMPLICIT INTEGER(I-N)
...
!
      subroutine bkmp2_f8(rin,Vtot,dVtot,id)
!-----------------------------------------------------------------
!  Calculate total H3 potential from all of its parts
!  if (id.gt.0) also calculate the dV/dr derivatives;
!  all distances are in bohrs and all energies are in hartrees.
!  Note that it is an error if any distance is less than 0.2 bohrs.
!
! Note: this file contains parameters for the 'BKMP2' H3 surface,
!       [re]fitted on june21/95 to a set of several thousand ab
!       initio points.  The '706' surface parameters have been
!       commented out (these were the parameters used in the
!       previous 'BKMP' surface).  The routine names have been
!       modified slightly (usually a '95' appended) so that a
!       program could easily call and compare both of our surfaces.
!
! Note: the surface parameter values as published lead to an
!       anomolously deep van der Waals well for a very compact
...
!       The old coefficients are still in subr.vbcb but have been
!       commented out.
!
...
!----------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      IMPLICIT INTEGER(I-N)
      dimension r(3),dVtot(3),rin(3)
      dimension dVlon(3),dVas(3),dVbnda(3),dVbndb(3), &
                dCal(3), dCas(3),dCbnda(3),dCbndb(3)
      dimension vb(2,25),cb(2,25)
!     dimension vbp(2,25),cbp(2,25)
!     dimension dB1a(3),dB1b(3)
      dimension dT(3),T(3)
!     dimension dB2(3),dB3(6),expass(4)

!      OPEN (70,FILE= 'LIXO2.TXT')

      ipr = 0

      r(1) = rin(1) / 5.2917721092d-11
       r(2) = rin(2) / 5.2917721092d-11
      r(3) = rin(3) / 5.2917721092d-11
!      print*,'no h3',r
      call chgeom_f8(r,ivalid)
      if(ivalid.lt.1)then
!-sngl;         vtot     = 99.0
         vtot     = 99.0d0
         dvtot(1) = 0.d0
...
!  zero the vb and cb arrays (used only for numerical derivatives)
      do i=1,25
       vb(1,i) = 0.d0
       vb(2,i) = 0.d0
       cb(1,i) = 0.d0
      ...
      call h3lond95_f8( r, Vlon, dVlon )
      call Vascal95_f8( r, Vas, dVas )
!    now do any corrections required for compact geometries:
      call compac95_f8(r,icompc,T,dT)
!    oct.3/90 compact routines only called for compact geometries:
      if(icompc.ge.1)then
         call csym95_f8  ( r, cal, dCal )
         call casym95_f8 ( r, Cas, dCas,  1 ,T,dT)
      end if
      call vbcb95_f8(r,icompc,T,dT,ipr, &
...
!      print*,'dVtot',dVtot
      ...
      return
 7400 format('Vba values: ',5(1x,g12.5),/,'Vbb values: ',5(1x,g12.5))
 7500 format('Cba values: ',3(1x,f16.9),/, &
             '            ',3(1x,f16.9),/, &
             '            ',3(1x,f16.9),/, &
             'Cbb values: ',3(1x,f16.9),/, &
             '            ',3(1x,f16.9),/, &
             '            ',3(1x,f16.9))
 7000 format(5x,'    r =',3(1x,f16.9),' icompac = ',i1,/, &
             5x,'Vtot  = ',f18.11,/, &
             5x,'Vlon  = ',f18.11,'       Vas   = ',g18.11,/, &
             5x,'Cal   = ',g18.11,'       Cas   = ',g18.11,/, &
             5x,'Vbnda = ',g18.11,'       Vbndb = ',g18.11,/, &
             5x,'Cbnda = ',g18.11,'       Cbndb = ',g18.11)
 7100 format(' dVtot   = ',3(1x,g18.11),/, &
             ' dVlon   = ',3(1x,g18.11),/, &
             ' dVas    = ',3(1x,g18.11),/, &
             ' dVbnda  = ',3(1x,g18.11),/, &
             ' dVbndb  = ',3(1x,g18.11),/, &
             ...
             ' dCbnda  = ',3(1x,g18.11),/, &
             ' dCbndb  = ',3(1x,g18.11))
 7750 format(3(1x,g16.9))
 7751 format(/,3(1x,g16.9))
...
!       WRITE(70,*)'from bkmp2_f8 to avoid paralelization erros'
!      CLOSE(70)
        end
!
      subroutine triplet95_f8(r,E3,id)
!--------------------------------------------------------------------
...
! triplet and johnson values mar08/95
!     parameter(
!    .  a1= -0.0460730469,a2=-16.3948753611,a3=-27.6026451962,
!    .  a4=  2.0383359014,a5= -6.8887433326,a6=  1.6216005938,
!    .  C1= -0.4110341110310669,C2= -0.0767398928636804,
!    .  C3=  0.4302169435606523)
! triplet values jun21
!-sngl;      parameter(
!-sngl;     .  a1= -0.0298546962,a2=-23.9604445036,a3=-42.5185569474,
!-sngl;     .  a4=  2.0382390988,a5=-11.5214861455,a6=  1.5309487826,
!-sngl;     .  C1= -0.4106358351531854,C2= -0.0770355790707090,
!-sngl;     .  C3=  0.4303193846943223) !jun21 fit
      parameter( &
...
!  triplet and johnson values from fit621:
!     parameter(a1=-0.0253496194,a2=-29.2302126444,
!    .          a3=-50.7225015503,a4= 2.0452676876 ,
!    .          a5=-12.2408908509,a6= 1.6733157383 )
!     parameter( C1=-0.4170298146519658,
!    .           C2=-0.0746027774843370,C3= 0.4297899952237434 )
! surface parameters from fit601.out
!     parameter(a1=-0.66129429,a2=-1.99434198,a3=-2.37604328,
!    .          a4= 2.08107802,a5=-0.0313032510,a6=3.76546699,
!    .          C1=-0.4222590135447196,C2=-0.0731117796738824,
!    .          C3= 0.4295918082189010 )
...
!
!
      subroutine vH2opt95_f8(r,E,ideriv)
!-----------------------------------------------------------------
...
!-----------------------------------------------------------------
      ...
      IMPLICIT INTEGER(I-N)
      dimension E(3)
!-sngl;      parameter(a0=0.03537359271649620,    a1= 2.013977588700072    ,
!-sngl;     .  a2= -2.827452449964767        ,    a3= 2.713257715593500    ,
!-sngl;     .  a4= -2.792039234205731        ,    a5= 2.166542078766724    ,
!-sngl;     .  a6= -1.272679684173909        ,    a7= 0.5630423099212294   ,
!-sngl;     .  a8= -0.1879397372273814       ,    a9= 0.04719891893374140  ,
!-sngl;     . a10= -0.008851622656489644     ,   a11= 0.001224998776243630 ,
      parameter( &
...
!
!  calculate second derivative if required:
      if(ideriv.ge.2)then
         r10  = r6*r4
         rr27 = rr26*rr2
         rr44 = rr43*rr4
         rr62 = rr6*rr6
         rr63 = rr62*rr6
!        general term: (i-1)*(i-2)*a(i)*r**(i-3),    i=0,16
...
!
      subroutine h3lond95_f8(r,Vlon,dVlon)
!----------------------------------------------------------------------
! version of may 12/90  ... derivatives corrected (0.5 changed to 0.25)
! calculates the h3 london terms and derivatives
! modified oct 7/89 to include eps**2 term which rounds off the
! cusp in the h3 potential which occurs at equilateral triangle
! configurations
!----------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      IMPLICIT INTEGER(I-N)
...
!     dimension dJ1(3),dJ2(3),dJ3(3)
      parameter( half=0.5d0, two=2.d0 , eps2=1.d-12 )
      ipr = 0
      do i=1,3
         call vH2opt95_f8(r(i),esing,2)
        e1(i) = esing(1)
       de1(i) = esing(2)
       call triplet95_f8(r(i),etrip,2)
...
!  calculate the derivatives with respect to r(i):
 ...
 7000 format('             r = ',3(1x,f12.5),/, &
             '            E1 = ',3(1x,f12.8),/, &
             '            E3 = ',3(1x,f12.8),/, &
             '          vlon = ',1x,f12.8)
 7100 format(13x,'q = ',3(1x,e12.5),/,13x,'j = ',3(1x,e12.5))
 7200 format('         dVlon = ',3(1x,g12.5))
 7400 format('from subr.london: ',/, &
             '  using r',i1,':   Esinglet=',3(1x,f12.8),/, &
             '         ',1x,'    Etriplet=',3(1x,f12.8))
 7410 format('         sumQ = ',g12.5,'        sumJ = ',g12.5)
 7420 format('         Vlon = ',f12.8,'      rootJt = ',g12.5)
      return
      end
!
      subroutine Vascal95_f8(rpass,Vas,dVas)
!------------------------------------------------------------------
...
! vasym values jun21
!-sngl;      parameter(
!-sngl;     . aa1=0.3788951192E-02, aa2=0.1478100901E-02,
!-sngl;     . aa3=-.1848513849E-03, aa4=0.9230803609E-05,
!-sngl;     . aa5=-.1293180255E-06, aa6=0.5237179303E+00,
!-sngl;     . aa7=-.1112326215E-02) !jun21 fit
      parameter( &
...
!--- vasym values mar08.95
!     parameter(
!    . aa1=0.3759731624E-02, aa2=0.1476254095E-02,
!    . aa3=-.1866759453E-03, aa4=0.9218646237E-05,
!    . aa5=-.1287906069E-06, aa6=0.5201790843E+00,
!    . aa7=-.1062909514E-02)
!--- Vasym values from fit632c ----------------------
!     parameter(
!    . aa1=0.3438222224E-02,aa2=0.1398145763E-02,
!    . aa3=-.1923999449E-03,aa4=0.9712737075E-05,
!    . aa5=-.1263794562E-06,aa6=0.5181432712E+00,
!    . aa7=-.9487002995E-03 )
      ipr = 0
      r1 = rpass(1)
      r2 = rpass(2)
...
!   calculate the Vas term first (eq.14 of truhlar/horowitz)
 ...
 7100 format('    from subr.vascal:   Vas  = ',  1x,g12.5, /, &
             '                        dS   = ',3(1x,g12.5),/, &
             '                        dVas = ',3(1x,g12.5))
      return
      end
!
      subroutine acalc95_f8(r1,r2,r3,A,dA)
!---------------------------------------------------------------------
!  version of may 1, 1990
!  based on equations from notes date april 4, 1990
!  calculate the A value and its derivatives
!           A  = dabs[ (r1-r2)*(r2-r3)*(r3-r1) ]
!---------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      IMPLICIT INTEGER(I-N)
      dimension dA(3)
 ...
 7100 format('Acalc enter ---------------------------------------',/, &
             '     A = ',f15.10,'   dA = ',3(1x,g12.5),/, &
             'Acalc exit ----------------------------------------')
      return
      end
!
      subroutine compac95_f8(r,icompc,T,dT)
!---------------------------------------------------------------
...
!  first see if this is a compact geometry:
      do i=1,3
       T(i) = 0.d0
       dT(i)= 0.d0
      end do
...
!
      subroutine casym95_f8( r,Cas,dCas,id,T,dT )
!---------------------------------------------------------------
...
! Casym values jun21
!-sngl;      parameter(
!-sngl;     . u1=0.2210243144E+00, u2=0.4367417579E+00, u3=0.6994985432E-02,
!-sngl;     . u4=0.1491096501E+01, u5=0.1602896673E+01, u6=-.2821747323E+01,
!-sngl;     . u7=0.4948310833E+00, u8=-.3540394679E-01, u9=-.3305809954E+01,
!-sngl;     .u10=0.3644382172E+01,u11=-.9997570970E+00,u12=0.7989919534E-01,
!-sngl;     .u13=-.1075807322E-02) !jun21 fit
      parameter( &
...
!---- Casym values mar08.95
!     parameter(
!    . u1=0.5120831287E+00, u2=0.1002277242E+01, u3=0.6850007419E-02,
!    . u4=-.2038751706E+01, u5=0.7027811909E+01, u6=-.4881767278E+01,
!    . u7=0.8801769106E+00, u8=-.6296419648E-01, u9=-.8125516783E+01,
!    .u10=0.6073964424E+01,u11=-.1451402523E+01,u12=0.1165084183E+00,
!    .u13=-.1176579871E-02)
!---- Casym values from fit633a
!     parameter(
!    . u1=0.1537481166E+00, u2=0.2745950036E+00, u3=0.7501206780E-02,
!    . u4=0.3119136023E+01, u5=0.9969170798E+00, u6=-.3373682823E+01,
!    . u7=0.6807215913E+00, u8=-.4920325491E-01, u9=-.3919467989E+01,
!    .u10=0.5085532326E+01,u11=-.1415264778E+01,u12=0.1138681785E+00,
!    .u13=-.1525367566E-02)
      ipr = 0
      cas = 0.d0
      dCas(1) = 0.d0
      dCas(2) = 0.d0
      dCas(3) = 0.d0
      call acalc95_f8(r(1),r(2),r(3),A,dA)
      A2 = A*A
!     if(A.eq.0.d0) return
      sumT = T(1) + T(2) + T(3)
      Sr   = r(1) + r(2) + r(3)
...
!    write out the series explicitly:
      ...
      return
 7400 format('subr.Cas:   series = ',g12.5,'     term1 = ',g12.5,/, &
             '             eterm = ',g12.5,'      Cas  = ',g12.5)
 7500 format('    derivatives: dPr = ',3(1x,g12.5),/, &
             '                 dT  = ',3(1x,g12.5),/, &
             '              dterm1 = ',3(1x,g12.5),/, &
             '              determ = ',3(1x,g12.5),/, &
             '             dseries = ',3(1x,g12.5),/, &
...
!
      subroutine csym95_f8(r,Cal,dCal)
!---------------------------------------------------------------
! version of apr12/95  950308 values
...
! Csym values jun21
!-sngl;      parameter(
!-sngl;     . v1=-.2071708868E+00, v2=-.5672350377E+00, v3=0.9058780367E-02) !jun21 fit
      parameter( &
       v1=-.2071708868d+00, v2=-.5672350377d+00, v3=0.9058780367d-02) !jun21 fit
!---- Csym values mar08/95
!     parameter(
!    . v1=-.2210049400E+00, v2=-.7054469608E+00, v3=0.4088898394E-02)
!---- Csym values from fit633a
!     parameter(
!    . v1=-.2070776049E+00, v2=-.5350898737E+00, v3=0.1011861942E-01)
      cal   = 0.d0
      ipr   = 0
      Sr    = r(1)+r(2)+r(3)
      Sr2   = Sr*Sr
...
!
      subroutine vbcb95_f8(rpass,icompc,T,dT,ipr, &
                        Vbnda,Vbndb,dVbnda,dVbndb, &
                        Cbnda,Cbndb,dCbnda,dCbndb)
!---------------------------------------------------------------
! apr12/95 surface950308 values added
...
!-sngl;      parameter(
!-sngl;     .a11=-.1838073394E+03,a12=0.1334593242E+02,a13=-.2358129537E+00,
!-sngl;     .a21=-.4668193478E+01,a22=0.7197506670E+01,a23=0.2162004275E+02,
!-sngl;     .a24=0.2106294028E+02,a31=0.4242962586E+01,a32=0.4453505045E+01,
!-sngl;     .a41=-.1456918088E+00,a42=-.1692657366E-01,a43=0.1279520698E+01,
!-sngl;     .a44=-.4898940075E+00,a51=0.1742295219E+03,a52=0.3142175348E+02,
!-sngl;     .a53=0.5152903406E+01) !jun21 fit
      parameter( &
...
! vbendb terms jun21
!-sngl;      parameter(
!-sngl;     .g11=-.4765732725E+02,g12=0.3648933563E+01,g13=-.7141145244E-01,
!-sngl;     .g21=0.1002349176E-01,g22=0.9989856329E-02,g23=-.4161953634E-02,
!-sngl;     .g24=0.9075807910E-03,g31=-.2693628729E+00,g32=-.1399065763E-01,
!-sngl;     .g41=-.1417634346E-01,g42=-.4870024792E-03,g43=0.1312231847E+00,
!-sngl;     .g44=-.4409850519E-01,g51=0.5382970863E+02,g52=0.4587102824E+01,
!-sngl;     .g53=0.1768550515E+01) !jun21 fit
      parameter( &
...
!    vbenda mar08/95   expterms= 0.52d0, 0.052d0, 0.79d0
!     parameter(
!    .a11=-.1779469255E+03,a12=0.1292511538E+02,a13=-.2284450520E+00,
!    .a21=-.4671094689E+01,a22=0.7810423901E+01,a23=0.2359968959E+02,
!    .a24=0.2293876132E+02,a31=0.4191795902E+01,a32=0.4456025797E+01,
!    .a41=-.1419862958E+00,a42=-.1667760578E-01,a43=0.1246997125E+01,
!    .a44=-.4777517290E+00,a51=0.1681125268E+03,a52=0.3067567046E+02,
!    .a53=0.4942041527E+01)
!    vbendb mar08/95
!     parameter(
!    .g11=-.4677295200E+02,g12=0.3587873970E+01,g13=-.7051720560E-01,
!    .g21=0.9549632701E-02,g22=0.9971623818E-02,g23=-.4040392363E-02,
!    .g24=0.9045746595E-03,g31=-.2746842691E+00,g32=-.1412945503E-01,
!    .g41=-.1337907240E-01,g42=-.6200741002E-03,g43=0.1257251291E+00,
!    .g44=-.4195584422E-01,g51=0.5287171694E+02,g52=0.4490366264E+01,
!    .g53=0.1735452866E+01)
!
! cbenda terms jun21/95
!-sngl;      parameter(
!-sngl;     .c11=0.1860299931E+04,c12=-.6134458037E+03,c13=0.7337207161E+02,
!-sngl;     .c14=-.2676717625E+04,c15=0.1344099415E+04,c21=0.1538913137E+03,
!-sngl;     .c22=0.4348007369E+02,c23=0.1719720677E+03,c24=0.2115963042E+03,
!-sngl;     .c31=-.7026089414E+02,c32=-.1300938992E+03,c41=0.1310273564E+01,
!-sngl;     .c42=-.6175149574E+00,c43=-.2679089358E+02,c44=0.5577477171E+01,
!-sngl;     .c51=-.3543353539E+04,c52=-.3740709591E+03,c53=0.7979303144E+02,
!-sngl;     .c61=-.1104230585E+04,c62=0.4603572025E+04,c63=-.5593496634E+04,
!-sngl;     .c71=-.1069406434E+02,c72=0.1021807153E+01,c73=0.6669828341E-01,
!-sngl;     .c74=0.4168542348E+02,c75=0.1751608567E+02,c81=0.9486883238E+02,
!-sngl;     .c82=-.1519334221E+02,c83=0.4024697252E+04,c84=-.2225159395E+02) !jun21
      parameter( &
...
!-sngl;      parameter(
!-sngl;     .d11=0.4203543357E+03,d12=-.4922474096E+02,d13=0.3362942544E+00,
!-sngl;     .d14=-.3827423082E+03,d15=0.1746726001E+03,d21=0.1699995737E-01,
!-sngl;     .d22=0.1513036778E-01,d23=0.2659119354E-01,d24=-.5760387483E-02,
!-sngl;     .d31=0.1020622621E+02,d32=0.1050536271E-01,d41=0.6836172780E+00,
!-sngl;     .d42=-.1627858240E+00,d43=-.6925485045E+01,d44=0.1632567385E+01,
!-sngl;     .d51=0.1083595009E+04,d52=0.4641431791E+01,d53=-.8233144461E+00,
!-sngl;     .d61=-.6157225942E+02,d62=0.3094361471E+03,d63=-.3299631143E+03,
!-sngl;     .d71=0.8866227120E+01,d72=-.1382126854E+01,d73=0.7620770145E-01,
!-sngl;     .d74=-.5145757859E+02,d75=0.2046097265E+01,d81=0.2540775558E+01,
!-sngl;     .d82=-.4889246569E+00,d83=-.1127439280E+04,d84=-.2269932295E+01) !jun21
      parameter( &
...
!    cbenda mar08/95
!     parameter(
!    .c11=0.1983833377E+04,c12=-.7161674985E+03,c13=0.9480354622E+02,
!    .c14=-.2860199829E+04,c15=0.1424701828E+04,c21=0.1544592401E+03,
!    .c22=0.3684848293E+02,c23=0.1717816399E+03,c24=0.2060270139E+03,
!    .c31=-.1215015279E+03,c32=-.1370574974E+03,c41=0.8903858200E+00,
!    .c42=-.6496543267E+00,c43=-.2367464822E+02,c44=0.5098769966E+01,
!    .c51=0.6728307258E+03,c52=-.3825078986E+03,c53=0.8128185587E+02,
!    .c61=-.1141379097E+04,c62=0.4771127549E+04,c63=-.5713570141E+04,
!    .c71=-.7394045711E+01,c72=0.9888694698E+00,c73=0.7493009017E-01,
!    .c74=0.9590644736E+02,c75=0.1925781563E+02,c81=0.1001295468E+03,
!    .c82=-.1533150643E+02,c83=-.1850297943E+03,c84=-.2420454635E+02)
!     (last 4 parameters renumbered)
!    cbendb terms mar08/95
!     parameter(
!    .d11=0.4441478933E+03,d12=-.5345816836E+02,d13=0.1443665553E+01,
!    .d14=-.4155134339E+03,d15=0.1912235377E+03,d21=-.3697316400E-02,
!    .d22=0.1688497284E-01,d23=0.2330055461E-01,d24=-.4670024282E-02,
!    .d31=0.6492099426E+01,d32=0.1597054504E-01,d41=0.6701003237E+00,
!    .d42=-.1868909419E+00,d43=-.6709596498E+01,d44=0.1713120242E+01,
!    .d51=-.5022064388E+04,d52=0.2322997491E+01,d53=-.2710031511E+00,
!    .d61=-.7265968242E+02,d62=0.3593588680E+03,d63=-.3933599161E+03,
!    .d71=0.9312252325E+01,d72=-.1380396384E+01,d73=0.7373026488E-01,
!    .d74=-.5070954743E+02,d75=0.7931122062E+00,d81=0.3599502276E+01,
!    .d82=-.5609868931E+00,d83=0.4980220561E+04,d84=-.1087179983E+01)
!     (last 4 parameters renumbered)
!
! Vbenda values from fit634b (used for surf706)
!     parameter(
!    .a11=-.2918252280E+03,a12=0.2164569141E+02,a13=-.4005497543E+00,
!    .a21=-.2890774947E+01,a22=0.1032032542E+02,a23=0.2681748056E+02,
!    .a24=0.2633751055E+02,a31=0.6180641351E+01,a32=0.5037667041E+01,
!    .a41=-.1125570079E+00,a42=-.3176529304E-02,a43=0.9068915355E+00,
!    .a44=-.7228418516E+00,a51=0.2785898232E+03,a52=0.4764442446E+02,
!    .a53=0.8621545662E+01)
!   Vbendb values from fit634b (used for surf706)
!     parameter(
!    .g11=-.4241912309E+02,g12=0.2951365281E+01,g13=-.4840201514E-01,
!    .g21=-.1159168549E-03,g22=0.8688003567E-02,g23=-.3486923900E-02,
!    .g24=0.8312212839E-03,g31=0.5621810473E-01,g32=-.9776930747E-02,
!    .g41=-.1178456251E-01,g42=0.3491086729E-02,g43=0.7430516993E-01,
!    .g44=-.9643636957E-01,g51=0.4735533782E+02,g52=0.3001038808E+01,
!    .g53=0.1896630453E+01)
!
! parameters required for Cbend calculations:
!
!-- Cbenda values from fit706d.out
!     parameter(
!    .c11=0.7107064647E+04,c12=-.3728421267E+04,c13=0.1757654624E+03,
!    .c14=-.9725998132E+04,c15=0.4665086074E+04,c21=-.4435165986E+02,
!    .c22=0.1604477309E+03,c23=0.5805142925E+03,c24=0.6892349445E+03,
!    .c31=0.6581730442E+03,c32=0.6078389739E+02,c41=0.2885182566E+01,
!    .c42=-.1728169916E+01,c43=-.1119535503E+03,c44=0.4052536250E+02,
!    .c51=0.2540505673E+03,c52=-.5762083627E+03,c53=0.1295901320E+03,
!    .c61=-.2131706075E+04,c62=0.9084452020E+04,c63=-.1138253963E+05,
!    .c71=-.3964298833E+02,c72=-.5019979693E+01,c73=0.2906541488E+00,
!    .c74=0.1212943686E+04,c75=0.4140463415E+02,c81=0.1752855549E+03,
!    .c82=-.2496320107E+02,c83=0.3765413052E+03,c84=-.5480488130E+02)
!-- Cbendb values from fit706d.out
!     parameter(
!    .d11=0.1917166552E+04,d12=-.6542563392E+03,d13=0.6793758367E+02,
!    .d14=-.1694968847E+04,d15=0.6866649703E+03,d21=-.2137567948E+00,
!    .d22=0.4975938228E-01,d23=0.9364998295E-01,d24=-.2444320779E-01,
!    .d31=-.2863126914E+02,d32=0.5443219625E-01,d41=0.9673956120E+00,
!    .d42=-.1160159706E+01,d43=-.2424199759E+02,d44=0.8569424490E+01,
!    .d51=-.6517635862E+04,d52=0.1518098147E+03,d53=-.2706514366E+02,
!    .d61=0.4308392956E+02,d62=-.1234851732E+03,d63=0.2320626055E+03,
!    .d71=0.1049541418E+02,d72=-.2424169341E+01,d73=0.1745646946E+00,
!    .d74=0.2603615561E+02,d75=0.1345799970E+02,d81=-.6653710513E+01,
!    .d82=-.2576854447E+00,d83=0.6172608425E+04,d84=-.1328142473E+02)
! end of parameters
!
...
!d    if(ipr.gt.1)then
!d        write(7,*)
!d        write(7,*) ' from subr.vbcb -----------------------------'
!d        write(7,6000) rpass
!d        write(7,6010) b1a,b1b
!d        write(7,6020) db1a,db1b
!d        if(ipr.gt.2)then
!d           write(7,7000) dc1dr1,dc1dr2,dc1dr3,
!d   .                     dc2dr1,dc2dr2,dc2dr3,
!d   .                     dc3dr1,dc3dr2,dc3dr3
!d           write(7,7010) t1,t2,t3,c1,c2,c3,d1,d2,d3
!d        end if
!d    end if
!
...
!d    if(ipr.gt.1)then
!d       write(7,7650) B2,B3
!d       write(7,7700) db2,db3
!d       write(7,*)
!d       write(7,*) 'Vbend values :'
!d       k=1
!d       write(7,7150) k,dvb1,dvb2,dvb3,dvb4,dvb5,dVbnda
!d       write(7,7155) (vb(1,i),i=1,5)
!d       k=2
!d       write(7,7150) k,dvb1,dvb2,dvb3,dvb4,dvb5,dVbndb
!d       write(7,7155) (vb(2,i),i=1,5)
!d       write(7,7100) Vbnda,Vbndb
!d       write(7,*)
!d       write(7,*) 'Cbend values:'
!d       write(7,*) 'c81,c81,c82=',c81,c81,c82
!d       write(7,*) 'c91,c83,c84=',c91,c83,c84
!d       write(7,*) 'd81,d81,d82=',d81,d81,d82
!d       write(7,*) 'd91,d83,d84=',d91,d83,d84
!d       k=1
!d          write(7,2020) k,(cb(1,j),j=1,9)
!d          write(7,2033) sumT,Cbnds(1)
!d       k=2
!d          write(7,2020) k,(cb(k,j),j=1,9)
!d          write(7,2033) sumT,Cbnds(2)
!d       k=1
!d       write(7,7152) k,dcb1,dcb2,dcb3,dcb4,dcb5,dcb6,
!d   .               dcb7,dcb8,dcb9,(dCbnds(1,i),i=1,3)
!d       k=2
!d       write(7,7152) k,dcb1,dcb2,dcb3,dcb4,dcb5,dcb6,
!d   .                 dcb7,dcb8,dcb9,(dCbnds(k,i),i=1,3)
!d       write(7,7200) dT,dCbnda,dCbndb
!d       write(7,*) 'VbCb  exit -------------------------'
!d       write(7,*)
!d    end if
      return
 2000 format(5x,'r1, r2, r3 = ',3(1x,f12.8))
 2010 format(5x,'B1a, B1b, B2, B3 = ',4(1x,g12.5))
 7100 format('    Vbnda = ',g12.5,'     Vbndb = ',g12.5)
 7650 format('    B2 =    ',g12.5,'       B3 =  ',g12.5)
 7700 format('    dB2 =   ',3(1x,g12.5),/, &
             '    dB3 =   ',6(1x,g12.5))
 7150 format(' k=',i1,'     derivatives: ',/, &
             ...
             ' dVb5  = ',3(1x,g16.9),/, &
             ' dVbnd = ',3(1x,g16.9))
 7155 format('  Vb1 = ',g16.9,'  Vb2 = ',g16.9,/, &
             '  Vb3 = ',g16.9,'  Vb4 = ',g16.9,/, &
             '  Vb5 = ',g16.9)
 2020 format(5x,'k=',i1,' cb1,cb2,cb3= ',3(1x,g16.9), &
           /,8x,' cb4,cb5,cb6= ',3(1x,g16.9), &
           /,8x,' cb7,cb8,cb9= ',3(1x,g16.9))
 2033 format(5x,'sumT = ',g12.5 ,'  cbnds(k) = ',g12.5)
 7152 format('Subr.Cbend:   k=',i1,'     derivatives: ',/, &
             ...
             '  dCb9  = ',3(1x,g18.11),/, &
             '  dCbnd = ',3(1x,g18.11))
 7200 format('  dT    = ',3(1x,g18.11),/, &
             '  dCbnda= ',3(1x,g18.11),/, &
             '  dCbndb= ',3(1x,g18.11))
 6000 format(' from subr.b1ab:  r=',3(1x,f9.6))
 6010 format('    b1a = ',f16.9,'     b1b = ',f16.9)
 6020 format('    db1a: ',3(1x,e12.5),/,'    db1b: ',3(1x,e12.5))
 7000 format('    dc(j)/dr(i) derivatives: ',/, &
          8x,3(1x,e12.5),/,8x,3(1x,e12.5),/,8x,3(1x,e12.5))
 7010 format('    ri2-rj2-rk2: ',3(1x,e12.5),/, &
             '       cosines : ',3(1x,e12.5),/, &
             '    12c**2 - 3 : ',3(1x,e12.5))
      end
!
      subroutine chgeom_f8(r,ivalid)
!----------------------------------------------------------------------c
! apr21/95
! first, check that it is a valid H3 geometry, within tolerance "derr":
! this test corrected on nov23/93
!     generally, rlo + rmed > rhi
!     sometimes, rlo + rmed = rhi (linear geometry)
!     but if     rlo + rmed < rhi there's a problem
!
! error1 ... check that rlo+rmid>rhi
! istop1 ... [0] warning [1]stop calculations
!
! error2 ... check that all r(i)'s are > 0.2 bohr
! istop2 ... [0] warning [1]stop calculations
!----------------------------------------------------------------------c
      implicit double precision (a-h,o-z)
      IMPLICIT INTEGER(I-N)
...
!x    if(rlo+rmid.gt.rhi+derr)then  {old condition}
      if(rlo+rmid+derr.lt.rhi)then
!d       write(6,*) ' rlongest          = ',rhi+derr
!d       write(6,*) ' rmiddle,rshortest = ',rmid,rlo
         write(6,*) 'Warning: invalid geometry ',r
         if(istop1.gt.0) stop ' STOP -- geometry error '
!         ivalid = -1
      end if
      if(rlo.lt.0.2d0)then
!d       write(6,*) ' shortest distance: ',rlo
         write(6,*) 'Warning: invalid geometry ',r
